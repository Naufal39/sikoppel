...
class beritaController extends Controller
{
    ...
    { //mencari single berita yang dikirim dari WEB dengan metode MODEL BINDING
        return view('beritadanisu/isiberita', [
            "title" => $berita->title,
            "post" => $berita,
            "sideberita" => Berita::latest()->filter(request(['search', 'category', 'user']))->limit(3)->get(),
            // "skipthis" => $berita->skip($berita)
        ]);
    }
}
